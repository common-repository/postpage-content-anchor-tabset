...
function w4_tabset_db_install() {
	...
	$sql = "CREATE TABLE IF NOT EXISTS {$wpdb->tabset} (
...
function w4_tabset_table_exists(){
	global $wpdb;
	return strtolower( $wpdb->get_var( "SHOW TABLES LIKE '$wpdb->tabset'" )) == strtolower( $wpdb->tabset );
}
...
function w4_tabset_admin_save_option(){
	...
	if( !isset( $_GET['page'] ) || $_GET['page'] != W4CT_SLUG )
		return;
...
function w4_tabset_admin_page(){ ?>
	<div id="w4ct_admin" class="wrap">
    	<div class="icon32" id="icon-post"><br/></div>
        <h2>W4 content tabset <?php echo "V-".W4CT_VERSION ; ?>
        	<span class="desc"> - <?php _e( 'Wordpress plugin for creating jquery tabs with shorcode inside posts and pages content. Also capable to show your custom field values in a post or page content area by shortcode.' ); ?></span>
			<p style="background-color:#FFFFE0; border:1px solid #E6DB55; padding:5px 10px; border-width:1px 0;"> - <?php 
			_e( sprintf( 'Looking for a post listing plugin ? With lots of customable parameters ? <a href="%s" target="_blank">Try W4 post list</a>. It\' customizable, very very customizable.', 'http://wordpress.org/extend/plugins/w4-post-list/') ); ?></p>
		</h2>
		...
		$tabset_msgs = array(
						...
						'tabset_created' 	=> __( 'New tabset style Created.'),
						'tabset_deleted'	=> __( 'One tabset style has been deleted.'),
...
function w4_tabset_admin_welcome_page(){
	...
	echo '<p>Click on the menu item above to edit a tabset or create button to create a new tabset.</p>';

	if( !is_wp_error( $wp_feed )){
		echo '<h4>Learn more about WordPress -</h4>';
...
function w4_tabset_admin_js(){	?>
	<script type="text/javascript">
            jQuery("#tabset_help").click(function() {
                jQuery("#contextual-help-wrap").slideToggle({ duration: 'fast' });
                jQuery("#contextual-help-wrap").css({ 'background-color':'#DDDDDD', 'border-style': 'solid', 'border:color':'#E6DB55', 'border-width':'medium'});
                jQuery('#contextual-help-link-wrap').toggle() ;
                return false;
            });
            
			jQuery("#tabset_form .option").bind({
				mouseover: function(){
					jQuery(".option input.save_w4_content_tabset_options").remove();
					jQuery(this).append('<input type="submit" name="save_w4_content_tabset_options" class="save_w4_content_tabset_options" value="Save option" />');
				}
			});
			
			jQuery('a#delete_list').click(function(){
...
function w4_tabset_help(){
		...
		'<li>' . __('For farther documentation contact <a href="mailto:workonbd@gmail.com" rel="tabset_author_mail">Shazzad</a> or visit plugin site <a href="http://w4dev.com/" class="us" rel="developer" title="Web and wordpress development...">W4DEV</a>..') . '</li>' .
		'</ul>' ;
		return $tabset_help ;
	}
...
function w4_tabset_menu( $current = 0){
	...
	$tabset_admin_menu = '<ul class="tabset_admin_menu">';
...
<div class="menu">
    ...
    _e( 'Web and wordpress development...' ); ?>">By W4DEV</a>
    <a href="http://w4dev.com/w4-plugin/post-page-custom-tabset-shortcode/?ref=admin_page_menu" target="_blank" title="<?php _e( 'Visit Plugin Page' ); ?>"> <?php 
    _e( 'Visit Plugin Page' ); ?></a>
    <a href="mailto:workonbd@gmail.com" rel="tabset_author_mail"> <?php _e( 'Mailto:Author' ); ?></a>
    <a href="http://wordpress.org/extend/plugins/postpage-content-anchor-tabset/" target="_blank" rel="wordpress"> <?php _e( 'Rate on WordPress' ); ?></a>
    <a id="tabset_help" style="background-color: #FFFFE0; border:1px solid #E6DB55;color: #000000;" class="" href="javascript:void(0);" title="<?php _e( 'Tabset documentation'); ?>"> Need help ?</a>
    <a href="<?php echo admin_url('edit.php?page='.W4CT_SLUG); ?>"><?php _e( 'Home' ); ?></a>
</div>
<?php
}
?>